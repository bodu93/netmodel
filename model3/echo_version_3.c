/*
 * prefork processes with pthreads-mutex-locked accept(2) call
 * codes from UNPv3
 */
#include <unistd.h>
#include <errno.h>
...
static pid_t pids[PROCESSESCOUNT];
...
void echo_sigint(int signo) {
	...
	if (errno != ECHILD) {
		/* NOTE: fprintf is not a async-signal-safe call */
...
int main() {
	...
	for (;;) {
		/* pause parent process until user press Ctrl+C */
		pause();
	}
